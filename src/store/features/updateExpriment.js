...
export const updateExperimentSlice = createSlice({
  ...
  reducers: {
    ...
    updateSwatchList: (state, action) => {
      let currentStatus = action?.payload?.length
    ...
    updateSwatchName: (state, action) => {
      ...
      const swatchToUpdate = state.swatches.find(
        (swatch) => swatch.swatch_id === swatchId
      );
      if (swatchToUpdate) {
    ...
    updateCurrentStep: (state, action) => {
      ...
      state.currentSwatchStatus = currentData?.find(
        (item) => item?.steps === action?.payload
      );
    },
...
export const initializeExperimentPage =
  (experiment_id) => async (dispatch, getState) => {
    ...
    if (data) {
      dispatch(
        updateCurrentExperiment({ ...data?.results, id: experiment_id })
      );
    }
    ...
    await dispatch(
      updateCurrentStep(swatchList?.[+swatchList?.length - 1]?.steps)
    );
  };
...
export const getSwatchByExperimentId =
  (experiment_id) => async (dispatch, getState) => {
    const { status, data } = await client.post("/get_swatch_data", {
      experiment_id,
    });
    if (status && data) {
...
export const updateSwatchPosition =
  ...
  async (dispatch, getState) => {
    ...
    dispatch(
      updateSwatchRank({
        swatch_id: updatedSwatches[index]?.swatch_id,
        new_rank: newPriority,
      })
    );
...
export const createSwatch = (swatch_name) => async (dispatch, getState) => {
  ...
  let currentSwatch = swatchLists.find(
    (item) => item?.swatch_id == data?.swatch_id
  );
...
export const deleteSwatch = (swatchId) => async (dispatch, getState) => {
...
const getSwatchList = () => async (dispatch, getState) => {
  ...
  if (status) dispatch(updateSwatchList(data?.results?.images || []));

  return data?.results?.images;
};

export const addSwatchImage =
  ({ isSameStep }) =>
  async (dispatch, getState) => {
    const currentSwatch = getState()?.updateExperiment?.activeSwatch;
    const currentData = getState()?.updateExperiment?.currentExperiment;
    ...
    const currentSwatchStatus =
      getState()?.updateExperiment?.currentSwatchStatus;
    ...
    if (currentStep === 1 || currentSwatchStatus?.wash_count !== washCount) {
      ...
      if (frontImage?.preview)
        bodyFormData.append(
          "front_image",
          dataURLtoFile(frontImage?.preview, "front.jpg")
        );
      if (backImage?.preview)
        bodyFormData.append(
          "back_image",
          dataURLtoFile(backImage?.preview, "back.jpg")
        );

      const { status, data, message } = await client.post(
        "/add_image_to_swatch",
        bodyFormData,
        { contentType: "multipart/form-data" }
      );
      if (!status) return toastr.error(message);
      ...
      const { status, data, message } = await client.put(
        "/update_swatch_image",
        updateBodyFormData,
        { contentType: "multipart/form-data" }
      );
    ...
    if (!isSameStep && (currentSwatchStatus?.steps || 1) == 3) {
      ...
      let current = swatches.find(
        (item) => item?.swatch_id == currentSwatch?.swatch_id
      );
    ...
    }
    {
      ...
      if (!isSameStep)
        await dispatch(
          updateCurrentStep((currentSwatchStatus?.steps || 1) + 1)
        );
    }
...
export const getExpSummary = () => async (dispatch, getState) => {
  ...
  if (status) dispatch(updateFinalResult(data?.mean_table));
};
